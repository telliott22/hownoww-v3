...
const CopuponScheduleTable: FunctionComponent<{ coupons: Coupon[] }> = function ({ coupons }) {
  return (
            ...
            coupons && coupons.length > 0
              ? coupons.map((coupon, index) => (
                <tr key={coupon.date} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td colSpan={1} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{numberWithCommas(parseFloat(coupon.principal))}</td>
                  <td colSpan={1} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{numberWithCommas(parseFloat(coupon.total))}</td>
                </tr>
              ))
